...
ife_setup_pte <- function(yname,
  ...
  time.periods <- sapply(original_time.periods, orig2t, original_time.periods = unique_t)
  groups <- sapply(original_groups, orig2t, original_time.periods = unique_t)


  # put the period in new time scale
  data$period <- sapply(data$period, orig2t, original_time.periods = unique_t)
  data$G <- sapply(data$G, orig2t, original_time.periods = unique_t)
